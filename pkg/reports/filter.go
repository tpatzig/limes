...
//ProjectFilter extends the Filter and describes query parameters
//to filter the project reports generated by this package.
...
func ReadProjectFilter(r *http.Request) ProjectFilter {
	...
	_, withSubresources := query["detail"]
	f.withSubResources = withSubresources

	queryValues, withRates := query["rates"]
	f.withRates = withRates

	onlyRates := false
	if withRates && util.StringSliceContains(queryValues, "only") {
		onlyRates = true
	}
